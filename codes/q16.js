var guestList = ["Ahmed", "Ahsan", "Hasaan", "Zayad"];
console.log("Unfortunately, the new dinner table won't arrive in time for the dinner.");
console.log("We can only invite two people for dinner.");
while (guestList.length > 2) {
    var removedGuest = guestList.pop();
    console.log("Sorry, ".concat(removedGuest, ", we won't be able to invite you to dinner."));
}
for (var _i = 0, guestList_1 = guestList; _i < guestList_1.length; _i++) {
    var guest = guestList_1[_i];
    console.log("Dear ".concat(guest, ", you're still invited to dinner. Please join us for an evening of engaging conversations."));
}
guestList.length = 0; // Remove the last two names from the list
console.log("Current guest list: ".concat(guestList));
